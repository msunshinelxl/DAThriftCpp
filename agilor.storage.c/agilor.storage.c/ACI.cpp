...
long aci::md_UnregisterDevice(string& deviceName, string* cause, long errCode)
{
	const char *c = NULL;
	if (cause != NULL) c = cause->data();
	return DRTDB_MD_UnregisterDevice(const_cast<LPSTR>(deviceName.data()), const_cast<LPSTR>(c), errCode);
}
